...
- **Cleaning a failed worktree session deletes the branch it made** (#173),
  ...
  — and only if it holds no commit its base lacks, counted again just
  before anything is removed. `--json` names it as `deleted_branch`. The
  popup's remove does the same, and its line says what remove deletes
  rather than "everything this create made", which was never true of the
  repository workspace herdr opens beside a first worktree.
