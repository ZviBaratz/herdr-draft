...
- `[clauth] launcher`: the argv that starts a pinned account, as a template
  ...
  than launching on an unintended account, and the reason is shown: on the
  `account` row's panel in the popup, and on stderr by `herdr-draft create`.
...
- `[clauth] launch` selects how a pinned account is launched, defaulting to
  ...
  That, a malformed `launcher` and an unrecognised `launch` are each reported,
  on the `account` row's panel in the popup and on stderr by
  `herdr-draft create`; a malformed `launcher` beside `"wrapper"` is reported
  as both malformed and ignored. The popup also reports the launch it
...
- **One thing reaches the network without being asked, and it is bounded.**
  ...
  the plugin makes runs with terminal prompts and ssh interaction disabled,
  because git asks for a password on `/dev/tty` rather than on the stdin it
  was handed, and inside the popup that is the terminal the form is drawn
  on. The fetch is additionally bounded at 30 seconds, since disabling
  prompts does not stop a configured askpass helper from opening a dialog
  that never answers. [`SECURITY.md`](SECURITY.md) states the same at
  greater length.
