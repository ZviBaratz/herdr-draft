...
- **`--dry-run` shows what a create would make, and makes nothing.** It
  ...
  ids (#209). It also carries `account_usage`, the one key a real create's
  object lacks: the usage windows of the account the session would bill,
  from clauth's status. The spawn skill weighs the model and effort against
