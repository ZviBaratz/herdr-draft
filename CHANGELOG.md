...
- **It refuses what the form refuses**, before anything is created (exit
  ...
  Its other non-`ok` value, `expired`, is an access token past its expiry
  whose refresh has not run yet: the `clauth start` this types hands the
  stored refresh token to `claude`, which refreshes it, so the popup marks
  it on the account row in the warning colour and neither path refuses it.
