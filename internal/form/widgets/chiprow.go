...
// unclipped (see widthStyle in picker.go).
//
// While inert, View renders only placeholder, dimmed, and nothing else.
// Otherwise it renders the chips separated by a dim "·" with the cursor
// chip highlighted, followed -- on a second line, only when non-empty -- by
// the FocusHint of the chip under the cursor: the generalized focused-hint
// mechanism ported from Atrium's chiprow.go:107-138 (see the file header).
func (c *ChipRow) View(width int) string {
	...
	for i, chip := range c.chips {
		label := " " + chip.Label + " "
		if i == c.cursor {
			row.WriteString(active.Render(label))
		} else {
			row.WriteString(plain.Render(label))
		}
		if i < len(c.chips)-1 {
