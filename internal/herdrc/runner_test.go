...
func TestCLIRunnerAwaitDetectionNotImplemented(t *testing.T) {
	r := &CLIRunner{Bin: "herdr"}
	if err := r.AwaitDetection(context.Background(), "w1:p1", time.Second); err == nil {
		t.Fatal("expected an error (not implemented), got nil")
	}
}

func TestCLIRunnerPaneRunNotImplemented(t *testing.T) {
	r := &CLIRunner{Bin: "herdr"}
	if err := r.PaneRun(context.Background(), "w1:p1", []string{"echo", "hi"}); err == nil {
		t.Fatal("expected an error (not implemented), got nil")
	}
}
