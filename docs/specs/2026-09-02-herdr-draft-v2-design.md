...
- **Loading.** The row reads `loading…` in dim; the panel says what is being
  ...
  > **Amended, draw-first spec §6 (#293, 2026-09-21).** That design is the
  > first to specify it, for `issue` (`waiting for api_key_cmd…`, `fetching
  > assigned issues…`) and `account` (`reading clauth profiles…`). The row
  > reads `loading…` only when there is nothing to pick. With a cached list
  > the row keeps its value, and the phase is on the panel.
...
- **Absent by design.** Linear unconfigured, or fewer than two clauth
  ...
  > **Amended, draw-first spec §3.2 and §4.2 (#293, 2026-09-21).** Linear
  > unconfigured stays a static check. Fewer than two clauth profiles is one
  > only when clauth's status file is fresh. Otherwise the row is drawn
  > `loading…`, and an answer of fewer than two profiles leaves it in place
  > with a reason.
...
> `resets_at`. A real create's object does not carry it: it is there to
> be weighed before approval, and a real create neither needs it nor reads
> clauth to get it. The profile is the pinned one, the dry pick's for
...
> an absent `account` means nothing is pinned. For `auto` the windows come
> from clauth too, not from the picker's own `usage` (which the form's
> `auto` row draws): one source keeps one shape, where the picker's is two
> unlabelled figures with reset times in no fixed format. So with
> `[clauth] enabled = false` an `auto` dry run names the pick and carries
> no usage. The key is absent whenever the usage cannot be said: clauth
> switched off, an agent other than claude, a degraded status, a profile
> clauth does not report or one with no windows, or a status that could
> not be read. The last is said on stderr
> unless clauth is simply not installed. The report gives numbers, not a
> verdict. The spawn skill applies the form's threshold,
> `clauth.WarnThreshold`, which a test holds its prose to. It never picks
...
> **Amended, draw-first spec §3, §4.2 and §5.6 (#293, 2026-09-21).** In the
> popup none of the three waits before the draw any more. A key failure with a
> cached issue list puts its reason on the panel rather than the row, and the
> list stays pickable. The budgets are unchanged, so the last sentence above
> still gives their reason, but there is no "before the draw" left for them to
> apply to.
