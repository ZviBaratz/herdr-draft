...
- **`TestAssembledForm_OpeningState` must loop over sizes.** The v2 defect
  ...
  > **Amended, draw-first spec §9.1 (#293, 2026-09-21).** The form gains a
  > second opening state, with the Linear key, clauth and the picker still
  > out, and it is pinned at the same four sizes.
