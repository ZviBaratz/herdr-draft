...
- **herdr's `auto_switch` is drawn in your dark theme.** It follows your
  terminal's light or dark appearance, which herdr's config file cannot
  tell herdr-draft. Use [`[palette]`](configuration.md#palette) if the
  result reads wrong.

- **The `terminal` theme is not adjusted for contrast.** It sends your
  terminal's own palette entries, so its colours are whatever your
  terminal's palette makes them, and herdr-draft cannot see them to check.
  Its focused row has no fill, only the `▌` and a bold value. If a colour
  reads wrong, change it in your terminal or in
