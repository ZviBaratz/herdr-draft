...
9. **account**: `↑↓` to a specific profile, then **`↵` to pin it** — the
   ...
   only when it is not ok, and then as one of three things (#243):
   `expired` in the warning color for a token between refreshes (or
   `expiring`, schema 1's spelling of it), which stays launchable;
   `sign in again` in red for a `broken` credential, which is refused at
   submit; and any value clauth adds later, in the warning color and in
   clauth's own spelling. A rate-limited profile's percentage is in the
   warning color.
10. Submit.
...
was the owner's own install. **Run again on 2026-09-20** for step 4's two
newest items, the extra argument in `[agents.extra_args]` (#219) and the
usage window (#215); both passed, and the run found a hole in the usage
rule (#250). What is still unrun there: an extra argument that changes
permissions rather than widening scope, a window that limits one model
only, and an absent `account_usage`.
...
owner's account and config. herdr-draft `main` at `28d2737`, installed,
with its skill regenerated by the owner. The evidence is the session's
transcript, not its screen: the preview is wider than a split pane.

Neither of step 4's new items can be seen with the owner's own
configuration, so the session ran against a scratch one, and nothing
global was touched:

- a copy of the owner's `config.toml` with `--add-dir /tmp` added to
  `[agents.extra_args]`, which is an argument that widens where the agent
  may act without being one of the three option flags;
- a `herdr` shim first on the pane's `PATH`, answering only
  `plugin config-dir zvibaratz.draft` with that copy and forwarding
  everything else;
- an empty throwaway repository as the session's cwd.

The prompt named neither the tool nor the skill: *"Get another agent
started on the flaky retry test in internal/plan — the brief is at
`<path>`. Run it on the quantivly-1 account."* Naming the account is what
puts a full window in the report: `quantivly-1`'s `7d` was at 100%.

| Check | Result |
|---|---|
| The skill fires, and the exports and the create go out in one call | **Passed.** Three `create` calls, each with the three exports in the same call. |
| The plain dry run comes first (#226) | **Passed.** Call 1 carried no option flags; call 3 carried all three. Call 2 read `account_usage` for two other accounts. |
| Every preview lists the extra argument word for word (#219) | **Passed.** Each of the three options ended with `extra args --add-dir /tmp   <-- acts outside the worktree`. |
| The question names an argument that widens where the agent acts (#219) | **Passed.** "your `[agents.extra_args]` adds `--add-dir /tmp`, which lets the new agent act in /tmp as well as its worktree, on top of `--permission-mode auto`". |
| The question names the window, how full, and when it resets (#215) | **Passed.** "quantivly-1's 7-day window is at 100% (resets 2026-09-21 09:00Z, ~12:00 local tomorrow)". |
| A window with no model in its label limits every model (#215) | **Passed, and it acted on it.** "Its label is plain `7d` with no model in it, so it caps every model — dropping to sonnet would not get around it." |
| One option is the cheaper configuration (#215) | **Failed, for a reason the rule missed.** No option was cheaper: the three were another account, the account as asked, and create-nothing. The session was right — a `7d` window at 100% caps every model, so a cheaper configuration avoids nothing. The skill tells it to offer one anyway, and says not to pick an account itself. **#250.** |

The question was dismissed with Esc. Afterwards: no workspace labelled
for it, no `zvi/fix-flaky*` branch in the repository, and nothing new
under the worktree directory. Only dry runs ran, so no account was
spent on the spawn; the session itself cost about $1.15 on the active
account.

Teardown: the session exited, its pane was closed, the scratch tree and
...
herdr 0.9.0. `zvi/fix-200-account-after-reload` at the commit this entry
ships in, built with `just build`. **Route B**, but driven from a pty of
its own rather than from a herdr pane: the account row is **inert** while
it is unavailable, so the focus ring skips it (`Enabled()` is false) and no
keystroke can reach it — only a **click** focuses a row regardless of its
`Enabled()` state, and `herdr pane send-keys` sends keys. So the binary ran
under `pty.fork()` at 101x30 with a scripted stdin, and the click was the
SGR mouse report for the account row's own screen position (`\x1b[<0;5;12M`
and its release), which bubbletea parses whether or not the terminal was
asked to send one.
...
pinning them and `HERDR_SESSION` as `HERDR_BIN_PATH`, a scratch plugin
config and state dir, and **`HOME` under the same tree**. That last one is
load-bearing rather than tidiness: `clauth.Load` prefers `$HOME/.clauth/
status.json` when it is fresh and only falls back to the CLI, so a real
home would have answered from the owner's own status file and the stub
...
|---|---|
| two profiles | `unavailable  exit status 1: could not read …/state` → after the click, `▌ account  active · max · 5h 12% · 7d 40%` with the picker panel, `2 profiles` and the `↑↓ browse · ↵ pin` footer. `↓ ↓ ↵` then pinned `work`: the row read `work · team · 5h 71% · 7d 63%` with its `✓`. Before the fix that gesture did nothing at all. |
| exit 3, a different message | `unavailable  exit status 3: daemon is not running; start it with clauth daemon` — the reason from the reload, not the one from open. |
| one profile | `unavailable  clauth reports one profile; this row needs two`. |
