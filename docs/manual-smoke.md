...
**Run on 2026-09-19** (Recorded runs): steps 2, 3 and 4 in two fresh
sessions. Step 1 was the owner's own install. The confirmation cannot be
read in full from a pane that short, and asking the session to write out
what it did can trip the model's safeguards. The session's own transcript
is the reliable record: `$CLAUDE_CONFIG_DIR/projects/<cwd>/<session>.jsonl`,
where `<session>` is `herdr agent get <pane>`'s `agent_session.value`. It
holds every Bash call and the exact `AskUserQuestion` input.
...
avoided the words "herdr-draft", "spawn", "plugin" and "skill", and each
confirmation was cancelled with Esc, so nothing was created.
...
| "Hand issue 215 … off to someone new … it should come back to me with a plan first" | **Passed in full.** It dry-ran twice: plainly first, which read `claude-opus-5[1m]` and `xhigh` with provenance `extra_args`, then with its choice. Every option was `plan`, with the user's own id quoted (`--model 'claude-opus-5[1m]'`): `high` first, then `xhigh`, then `sonnet` at `high`. The recommended option's description read "Your default is xhigh". It passed `--no-reap` though `mark_ready` was off, which is harmless. |

The miss is the reason §7 now numbers its three steps and says why the
first dry run cannot be folded into the second. One of three readers
skipped it: the #209 tabletop and the second session both followed the
old text. Whether the numbered text holds in a real session is unrun.
